...
class PigTruckArrivalRepo:
    def __init__(self, table: Table):
        self.table = table

    def put(self, arrival: PigTruckArrival) -> None:
        self.table.put_item(Item=serialize_to_dict(arrival))

    def list(self, pk: str) -> list[PigTruckArrival]:
        response = self.table.query(
            KeyConditionExpression=Key("pk").eq(pk) & Key("sk").begins_with("PigTruckArrival|"),
